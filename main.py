...
dataset_simt_path = data_path + dataset+"simt/"
ratio=args.seed
if os.path.exists(dataset_simt_path) == False:
    os.makedirs(dataset_simt_path)
...
def get_embedding():
    inputs = [adj_matrix,adj_features,rel_features,time_dict]
    model.eval()
    with torch.no_grad():
